...
export const CheckoutModal: React.FC<CheckoutModalProps> = ({ 
  ...
  const { deliveryCost, taxAmount, total } = useMemo(() => {
    const isService = item.category.toUpperCase() === 'SERVICES';
    const delivery = isService ? 0 : Math.floor(2000 + (Math.sqrt(Math.pow(userPos.x - item.coordinates.x, 2) + Math.pow(userPos.y - item.coordinates.y, 2)) * 150));
    const subtotal = finalPrice + delivery;
    const tax = subtotal * TAX_RATE;
    return { deliveryCost: delivery, taxAmount: tax, total: subtotal + tax };
  }, [item, finalPrice, userPos.x, userPos.y]);
      ...
      <div className="w-full max-w-md bg-white text-black rounded-[4rem] p-12 space-y-10 shadow-2xl relative overflow-hidden">
        ...
        {step === 'summary' && (
          <div className="space-y-10 animate-fade-in">
            <div className="text-center space-y-2">
              <h2 className="text-3xl font-black italic tracking-tighter">Review Order</h2>
              <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest italic">Checkout Protocol</p>
            </div>
            ...
            <div className="space-y-4 border-t border-gray-100 pt-8">
              <div className="flex justify-between items-center text-sm">
                ...
                <span className="font-black tabular-nums">{formatNairaKobo(finalPrice)}</span>
              </div>
              {deliveryCost > 0 && (
                <div className="flex justify-between items-center text-sm">
                  <span className="text-gray-400 font-bold uppercase tracking-widest text-[9px]">Delivery Fee</span>
                  <span className="font-black tabular-nums">{formatNairaKobo(deliveryCost)}</span>
                </div>
              )}
              <div className="flex justify-between items-center text-sm">
                <span className="text-gray-400 font-bold uppercase tracking-widest text-[9px]">Tax (7.5%)</span>
                <span className="font-black tabular-nums">{formatNairaKobo(taxAmount)}</span>
              </div>
              <div className="flex justify-between items-center pt-6 border-t border-black/10">
                ...
                <span className="text-3xl font-black italic tracking-tighter tabular-nums">{formatNairaKobo(total)}</span>
              </div>
            </div>

            <div className="bg-gray-50 p-8 rounded-[2.5rem] border border-gray-100 space-y-4">
              <div className="flex justify-between items-center">
                <span className="text-[10px] font-black uppercase text-gray-400 tracking-[0.2em]">Wallet Balance</span>
                <span className={`text-sm font-black tabular-nums ${hasSufficientFunds ? 'text-teal-600' : 'text-rose-500'}`}>
                  {formatNairaKobo(balance)}
                </span>
              </div>
              {!hasSufficientFunds && (
            ...
            {hasSufficientFunds ? (
              <button 
                ...
                className="w-full py-7 bg-black text-white rounded-full font-black uppercase tracking-[0.4em] text-[11px] shadow-2xl active:scale-95 transition-all"
              >
                Pay with Wallet
              </button>
            ) : (
              <button 
                ...
                className="w-full py-7 bg-amber-500 text-black rounded-full font-black uppercase tracking-[0.4em] text-[11px] shadow-2xl active:scale-95 transition-all"
              >
                Top Up Now
              </button>
            )}
        ...
        {step === 'processing_wallet' && (
          <div className="py-24 flex flex-col items-center justify-center space-y-12 animate-fade-in">
            ...
            <p className="text-base font-black uppercase tracking-[0.3em] animate-pulse">Confirming Payment...</p>
          </div>
        )}

        {step === 'success' && (
          <div className="py-24 flex flex-col items-center justify-center space-y-10 animate-scale-up">
            <div className="w-32 h-32 bg-teal-500 text-white rounded-full flex items-center justify-center shadow-2xl">
            ...
            <div className="text-center space-y-3 px-6">
              <h2 className="text-5xl font-black italic tracking-tighter leading-none">Order Paid!</h2>
              <p className="text-[11px] text-gray-500 font-black uppercase tracking-widest leading-relaxed">Payment verified. Transaction tax included.</p>
            </div>
          </div>
        )}
      </div>
