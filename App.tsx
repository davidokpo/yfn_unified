...
const App: React.FC = () => {
  ...
  const addTransaction = (amount: number, label: string, type: 'incoming' | 'outgoing', category: Transaction['category'] = 'reward') => {
    ...
    >
      ...
      <header className="fixed top-8 inset-x-8 z-50 h-16 flex items-center justify-between pointer-events-none">
        ...
        >
          ...
          <div className={`
            px-3 py-1 rounded-full flex items-center gap-2 ml-2 transition-all duration-500 border
            ${activeCloudMode === 'active' ? 'bg-[#039be5]/20 border-[#039be5]/40 text-[#039be5]' : 
              activeCloudMode === 'local' ? 'bg-amber-500/20 border-amber-500/40 text-amber-500' : 
              'bg-gray-500/20 border-gray-500/40 text-gray-500'}
          `}>
            <div className={`w-1.5 h-1.5 rounded-full shadow-[0_0_8px_currentColor] ${isConnecting ? 'animate-pulse' : ''} bg-current`}></div>
            <span className="text-[8px] font-black uppercase tracking-widest">
              {isConnecting ? 'Syncing...' : activeCloudMode === 'active' ? 'Cloud Active' : 'Local Sovereignty'}
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 pointer-events-auto">
          <button 
            onClick={() => setCurrentView(ViewType.WALLET)}
            className="flex items-center gap-3 px-6 py-2.5 bg-white/5 border border-white/10 backdrop-blur-xl rounded-full hover:bg-white/10 transition-all shadow-xl spring-pop"
          >
             <span className="text-sm font-black text-amber-500 tracking-tighter tabular-nums">
               {formatNaira(balance)}
             </span>
      ...
      <main className="flex-1 pt-32 relative z-10">
        ...
        {currentView === ViewType.MARKETPLACE && (
          <MarketplaceView 
            balance={balance} 
            initialItem={initiallySelectedItem}
            wishlist={wishlist}
            collections={collections}
            onReward={(amt, label) => addTransaction(amt, label, 'incoming', 'referral')} 
            onPurchase={(amt, label) => addTransaction(amt, label, 'outgoing', 'purchase')}
            onViewItem={trackView}
